...
object RasterArea {
   ...
   * @return
   */
  def getGdcRasterArea(sc: SparkContext, cubeId: String, productName: String, bbox: String, startTime: String, endTime: String, aggregate: String, measurementsStr: String, outputDir: String): String = {
    ...
    val extentStr = timeSeriesStitches(0).extent.xmin.formatted("%.2f") + "_" + timeSeriesStitches(0).extent.ymin.formatted("%.2f") + "_" + timeSeriesStitches(0).extent.xmax.formatted("%.2f") + "_" + timeSeriesStitches(0).extent.ymax.formatted("%.2f")
    val outputPath = executorOutputDir + extentStr + "_result.TIF"
  ...
  def main(args: Array[String]): Unit = {
    ...
    getGdcRasterArea(sc, cubeId, rasterProductName, extent, startTime, endTime, aggregate, measurements, outputDir)
