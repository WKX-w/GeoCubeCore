...
import geotrellis.raster.{ByteConstantNoDataCellType, FloatConstantNoDataCellType, Raster, ShortConstantNoDataCellType, Tile, TileLayout, UByteCellType, UByteConstantNoDataCellType, UShortCellType, UShortConstantNoDataCellType}
import org.apache.spark.{SparkConf, SparkContext}
import org.apache.spark.rdd.RDD
import whu.edu.cn.application.gdc.gdcCoverage.{getCropExtent, getMultiBandStitchRDD, getSingleStitchRDD, scaleCoverage, stitchRDDWriteFile}
import whu.edu.cn.core.entity.{GcDimension, QueryParams, RasterTileLayerMetadata, SpaceTimeBandKey}
import whu.edu.cn.core.raster.query.DistributedQueryRasterTiles.getRasterTileRDD
import geotrellis.raster.mapalgebra.local._
import whu.edu.cn.util.NetcdfUtil.{isAddDimensionSame, rasterRDD2Netcdf}
import whu.edu.cn.util.{GcConstant, PostgresqlService}
import geotrellis.spark._
import whu.edu.cn.application.gdc.GDCTrigger.isOptionalArg
...
object RasterCubeFun {
  ...
  def loadCube(sc: SparkContext, cubeName: String, extent: String = null, startTime: String = null, endTime: String = null,
               properties: String = null, queryParams: QueryParams = new QueryParams(), scaleSize: Array[java.lang.Integer] = null, scaleAxes: Array[java.lang.Double] = null,
    ...
    if (queryParams.getCubeId == "" || queryParams.getRasterProductNames.isEmpty) {
      ...
      if (properties != null) {
        val propertyArray:Array[String] = properties
          .stripPrefix("[").stripSuffix("]")
          .split(",")
          .map(_.trim.stripPrefix("\"").stripSuffix("\""))
        queryParams.setMeasurements(propertyArray)
      }
    }
    ...
    //    if (scaleSize != null) {
    //      val queryLons = queryParams.extentCoordinates(2) - queryParams.extentCoordinates(0)
    //      val queryLats = queryParams.extentCoordinates(3) - queryParams.extentCoordinates(1)
    //      val extentLons = rasterTileLayerRdd._2.tileLayerMetadata.extent.xmax - rasterTileLayerRdd._2.tileLayerMetadata.extent.xmin
    //      val extentLats = rasterTileLayerRdd._2.tileLayerMetadata.extent.ymax - rasterTileLayerRdd._2.tileLayerMetadata.extent.ymin
    //      scaleSize(0) = scala.math.ceil((scaleSize(0) / queryLons) * extentLons).toInt
    //      scaleSize(1) = scala.math.ceil((scaleSize(1) / queryLats) * extentLats).toInt
    //      val actualCols = scala.math.ceil(rasterTileLayerRdd._2.tileLayerMetadata.layout.tileLayout.layoutCols * extentLons / (rasterTileLayerRdd._2.tileLayerMetadata.layout.extent.xmax - rasterTileLayerRdd._2.tileLayerMetadata.layout.extent.xmin)).toInt
    //      val actualRows = scala.math.ceil(rasterTileLayerRdd._2.tileLayerMetadata.layout.tileLayout.layoutRows * extentLats / (rasterTileLayerRdd._2.tileLayerMetadata.layout.extent.ymax - rasterTileLayerRdd._2.tileLayerMetadata.layout.extent.ymin)).toInt
    //      scaleSize(0) = scala.math.ceil(scaleSize(0).toDouble / actualCols).toInt
    //      scaleSize(1) = scala.math.ceil(scaleSize(1).toDouble / actualRows).toInt
    //    }
    ...
    //      //      val preTileLayerMetadata = rasterTileLayerRdd._2.tileLayerMetadata
    //      //      val newLayout = new LayoutDefinition(rasterTileLayerRdd._2.tileLayerMetadata.extent,
    //      //        new TileLayout(rasterTileLayerRdd._2.tileLayerMetadata.layout.tileLayout.layoutCols,
    //      //          rasterTileLayerRdd._2.tileLayerMetadata.layout.tileLayout.layoutRows, scaleSize(0), scaleSize(1)))
    //      //      rasterTileLayerRdd._2.setTileLayerMetadata(new TileLayerMetadata[SpaceTimeKey](preTileLayerMetadata.cellType, newLayout, preTileLayerMetadata.extent, preTileLayerMetadata.crs, preTileLayerMetadata.bounds))
    //    }
    rasterTileLayerRdd
  }
  ...
  def mathCalculate(tile1: Tile, tile2: Tile, method: String, maxMin: ((Double, Double), (Double, Double))): Tile = {
    ...
    method match {
      ...
      case "divide" =>
        tileType = FloatConstantNoDataCellType
      case "normalize" =>
        tileType = FloatConstantNoDataCellType
    }
    if ((!tile1.isNoDataTile) && (!tile2.isNoDataTile)) {
      method match {
        ...
        case "divide" =>
          val tileDouble1 = tile1.convert(FloatConstantNoDataCellType)
          val tileDouble2 = tile2.convert(FloatConstantNoDataCellType)
        ...
        case "normalize" =>
          ...
          val tileN1 = tile1.convert(FloatConstantNoDataCellType)
          val tileN2 = tile2.convert(FloatConstantNoDataCellType)
  ...
  def normalizeOneTile(tile: Tile): Tile = {
    val tileD = tile.convert(FloatConstantNoDataCellType)
