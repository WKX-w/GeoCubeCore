...
import whu.edu.cn.conf.SparkCustomConf
import whu.edu.cn.core.entity.{QueryParams, RasterTileLayerMetadata, SpaceTimeBandKey, WorkflowCollectionParam}
...
object GDCTrigger {
  ...
  def func(implicit sc: SparkContext, UUID: String, funcName: String, args: mutable.Map[String, String]): Unit = {
    funcName match {
      case "loadCube" => {
        ...
        val cubeRDD: (RDD[(SpaceTimeBandKey, Tile)], RasterTileLayerMetadata[SpaceTimeKey]) = RasterCubeFun.loadCube(sc, cubeName = isOptionalArg(args, "cubeName"), extent = isOptionalArg(args, "extent"),
          startTime = isOptionalArg(args, "startTime"), endTime = isOptionalArg(args, "endTime"), properties = isOptionalArg(args, "properties"), queryParams = collectionQueryParams,
          scaleSize = scaleSize, scaleAxes = scaleAxes, scaleFactor = scaleFactor)
        if (cubeRDD == null) {
          isExceed = true
        } else {
          cubeRddList += (UUID -> cubeRDD)
        }
      }
      case "normalize" => {
    ...
    funcName match {
      ...
      case "normalize" => {
        ...
        if (dimensionMembersStr != null) {
          ...
          metaObject.put("usedBands", dimensionMembers.mkString(","))
        }
  ...
  def lambda(implicit sc: SparkContext, list: mutable.ArrayBuffer[Tuple3[String, String, mutable.Map[String, String]]]): Unit = {
    ...
    for (i <- list.indices if flag) {
      if (!isExceed) {
        ...
        paramObj.put("processName", this.processName)
        paramObj.put("jobId", this.jobId)
        println("JobId: " + this.jobId)
        println("ProcessName: " + this.processName)
        println("ParamJson: " + paramObj.toString)
  ...
  def main(args: Array[String]): Unit = {
    workTaskJson = {
      val fileSource: BufferedSource = Source.fromFile("src/main/resource/workflow4.json")
      val line: String = fileSource.mkString
      fileSource.close()
      line
    } // 任务要用的 JSON,应当由命令行参数获取
    //     val myArray: Array[Integer] = Array.ofDim[Integer](2)
    //      myArray(0) =259
    //      myArray(1) =259
    //      scaleSize = myArray
    dagId = Random.nextInt().toString
  ...
  def runWorkflow(implicit sc: SparkContext, workTaskJson: String, outputDir: String,
                  isCollection: String, workflowCollectionParamStr: String, processName: String, jobId: String): Unit = {
    this.scaleSize = null
    this.scaleAxes = null
