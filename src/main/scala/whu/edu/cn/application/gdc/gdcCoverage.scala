package whu.edu.cn.application.gdc
...
import geotrellis.raster.{CellGrid, CellType, MultibandTile, Raster, Tile, UShortConstantNoDataCellType}
import geotrellis.raster.io.geotiff.{GeoTiff, GeoTiffOptions, InterleaveMethod, MultibandGeoTiff, SinglebandGeoTiff, StorageMethod, Striped, Tags, Tiled}
...
import whu.edu.cn.core.entity._
import whu.edu.cn.core.raster.query.DistributedQueryRasterTiles.getRasterTileRDD
import whu.edu.cn.util.{GcConstant}
import whu.edu.cn.util.HttpRequestUtil.sendPost
import whu.edu.cn.util.NetcdfUtil.{isAddDimensionCombineSame, isAddDimensionSame, rasterRDD2Netcdf}
...
object gdcCoverage {
  ...
  def getCoveragePng(sc: SparkContext, queryParams: QueryParams, outputDir: String, imageFormat: String,
    ...
    val rasterTileLayerRdd: (RDD[(SpaceTimeBandKey, Tile)], RasterTileLayerMetadata[SpaceTimeKey]) = getRasterTileRDD(sc, queryParams, format = imageFormat, isGetData = true)
    if (rasterTileLayerRdd == null) {
      return null
    }
    ...
    if (imageFormat.equals("netcdf")) {
      println("The nc situation")
      rasterRDD2Netcdf(measurements, rasterTileLayerRdd, queryParams, scaleSize, scaleAxes, scaleFactor, outputDir)
    } else {
      println("The tif situation")
      // 多余维度只取第一个
  ...
  def stitchRDD2ImageFile[T <: CellGrid[Int]](queryParams: QueryParams, scaleSize: Array[java.lang.Integer],
    ...
    stitchedTile match {
      ...
      //        stitchRDDWriteFileTile(Raster(stitchedSingleTile, stitchedExtent), outputDir, imageFormat, stitchedExtent, crs)
  ...
  def getSingleStitchRDD(rasterTileLayerRdd: (RDD[(SpaceTimeBandKey, Tile)], RasterTileLayerMetadata[SpaceTimeKey])): Raster[Tile] = {
    val rasterTileRdd: RDD[(SpatialKey, Tile)] = rasterTileLayerRdd.map(x => (x._1.spaceTimeKey.spatialKey, x._2))
    val tileLayerArray: Array[(SpatialKey, Tile)] = rasterTileRdd.collect().filter(t => (t._1.col >= 0 && t._1.row >= 0))
    val (tile, (_, _), (_, _)) = TileLayoutStitcher.stitch(tileLayerArray)
    Raster(tile, rasterTileLayerRdd._2.tileLayerMetadata.extent)
  ...
  def getMultiBandStitchRDD(measurements: ArrayBuffer[String], rasterTileLayerRdd: (RDD[(SpaceTimeBandKey, Tile)], RasterTileLayerMetadata[SpaceTimeKey])): Raster[MultibandTile] = {
    val rasterMultiBandTileRdd: RDD[(SpaceTimeKey, MultibandTile)] = rasterTileLayerRdd.map(x => (x._1.spaceTimeKey, x._1.measurementName, x._2)).groupBy(_._1).map {
      x => {
        val tilePair = x._2.toArray.sortBy(t => measurements.indexOf(t._2))
        val multiBandTiles: ArrayBuffer[Tile] = new ArrayBuffer[Tile]()
        val cellTypes: Set[CellType] = tilePair.map(_._3.cellType).toSet
        ...
        //        }
        //        }
        // 检查是否存在多种不同的 cellType
        if (cellTypes.size > 1) {
          tilePair.foreach { ele =>
            // 进行数据类型转换为目标 cellType  硬编码
            multiBandTiles.append(ele._3.convert(UShortConstantNoDataCellType))
          }
        } else {
          // 如果只有一种 cellType，直接添加到 multiBandTiles
          multiBandTiles ++= tilePair.map(_._3)
        }
  ...
  def stitchRDDWriteFile[T <: CellGrid[Int]](stitchedTile: T, outputDir: String, imageFormat: String, stitchedExtent: Extent, crs: CRS): String = {
    ...
    if (imageFormat.equals("tif")) {
      val storageMethod: StorageMethod = Striped(stitchedTile.cols)
      val options: GeoTiffOptions = GeoTiffOptions(storageMethod)
      stitchedTile match {
        case t: Tile =>
          SinglebandGeoTiff(t, stitchedExtent, LatLng, Tags.empty, options).write(outputImagePath, optimizedOrder = true)
        //          GeoTiffWriter.write(a, outputImagePath, optimizedOrder = true)
        //          GeoTiff(t, stitchedExtent, crs).write(outputImagePath)
        case mt: MultibandTile =>
          MultibandGeoTiff(mt, stitchedExtent, LatLng, Tags.empty, options).write(outputImagePath, optimizedOrder = true)
  ...
  def main(args: Array[String]): Unit = {
    ...
    //    val cubeId = "33"
    //    val rasterProductName = "hsvs"
    //    val extent = "-26.9,33.11,44.9,73.392"
    //    //    val extent = "-30,32,48,74"
    //    val startTime = "2020-09-14 23:59:59"
    //    val endTime = "2020-09-16 10:00:01"
    //    val bbox = "null args"
    //    val coordinates = "null args"
    //    val measurements: Array[String] = Array("Geopotential", "Temperature", "U-velocity", "V-velocity", "Specific_humidity"
    //    , "Vertical_velocity", "Vorticity_relative", "Divergence", "Relative_humidity")
    //    //    val scaleSize: Array[java.lang.Integer] = Array[java.lang.Integer](259, 259)
    ////////////////////////////////////////////////////////////////

    ///////////////////////sentinel-2 - test //////////////////////////////////////
//    val cubeId = "37"
//    val rasterProductName = "SENTINEL-2 Level-2A MSI"
//    val extent = "4.343943692628017,51.83556167716492,4.71025807838505,51.94223503640242"
...
//    val endTime = "2019-01-10 00:00:00"
//    //    val measurements: Array[String] = Array("AOT", "B3", "B4", "B8", "MSK_CLDPRB", "MSK_SNWPRB", "QA10", "TCI_B", "TCI_G", "TCI_R")
//    //    val measurements: Array[String] = Array("QA10", "TCI_G")
//    val measurements: Array[String] = Array("B3", "B4")
//    val bbox = "null args"
//    val coordinates = "null args"
    ////////////////////////////////////////////////////////////////
    ...
    val outputPath = getCoveragePng(sc, queryParams, outputDir, imageFormat, null,
      null, null)
    if (outputPath == null) {
      println("-----------Exceed limit----------")
      val paramObj: JSONObject = new JSONObject();
      paramObj.put("processName", "Coverage")
      paramObj.put("jobId", "sssssssss")
      sendPost(GcConstant.callBackRoot + "exceedLimit", paramObj.toString)
    }
  }


  def runGetCoverage(implicit sc: SparkContext, outputDir: String, collectionParamStr: String, jobId: String): Unit = {
    //    val conf = new SparkConf()
    //      .setAppName("Get Coverage")
    //      .set("spark.serializer", "org.apache.spark.serializer.KryoSerializer")
    //      .set("spark.kryo.registrator", "geotrellis.spark.store.kryo.KryoRegistrator")
    ...
    val outputPath: String = getCoveragePng(sc, queryParams, outputDir, workflowCollectionParam.getImageFormat, scaleSize,
      scaleAxes, scaleFactor)
    if (outputPath == null) {
      println("-----------Exceed limit----------")
      val paramObj: JSONObject = new JSONObject();
