...
import whu.edu.cn.util.TileUtil
...
object ValueArea {
   ...
   * Used bu OGC DAPA value area endpoint.
   *
   * @param sc
   * @param productName
   * @param bbox
   ...
   * @return
   */
  def getDapaValueArea(sc: SparkContext, cubeId: String, productName: String, bbox: String, startTime: String, endTime: String, aggregate: String, measurementsStr: String, outputDir: String): String = {
    ...
    val timeResults = temporalGroupRdd.flatMap { x =>
      ...
      if (requestedExtent.intersects(srcExtent)) {
        ...
        aggregate match {
          case "min" => {
            val minMultibandPixelValue = new ArrayBuffer[(String, Float)]()
            (0 until measurements.length).foreach{i =>
              ...
              minMultibandPixelValue.append(( measurements(i), minPixelValue))
            }
            minMultibandPixelValue
          }
          case "max" => {
            val maxMultibandPixelValue = new ArrayBuffer[(String, Float)]()
            (0 until measurements.length).foreach{i =>
              val tile = cropMultibandTile.band(i)
              ...
              maxMultibandPixelValue.append((measurements(i), maxPixelValue))
            }
            maxMultibandPixelValue
          }
          case "avg" => {
            val avgMultibandPixelValue = new ArrayBuffer[(String, Float)]()
            (0 until measurements.length).foreach{i =>
              val tile = cropMultibandTile.band(i)
              var sumPixelValue: Float = 0.0f
              tile.foreachDouble{pixelValue => sumPixelValue += pixelValue.toFloat}
              avgMultibandPixelValue.append((measurements(i), sumPixelValue/(tile.rows*tile.cols)))
            }
  ...
  def main(args: Array[String]): Unit = {
    ...
    getDapaValueArea(sc, cubeId, rasterProductName, extent, startTime, endTime, aggregate, measurements, outputDir)
