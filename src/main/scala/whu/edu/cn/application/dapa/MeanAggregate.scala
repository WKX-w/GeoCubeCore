...
/**
 * Generate mean fields value from bands.
 *
 *
 */
object MeanAggregate {
  /**
    * Use List[(SpaceTimeKey,Tile)] as input
    * find the mean pixel of each tile
    * @param TileRDDList
    * @return tile
    */
  def generateMaxTile(TileRDDList:List[(SpaceTimeKey, Tile)]):Tile = {
    Mean(TileRDDList.map({x=>x._2}))
  }
  /**
   * This Mean function is used in Jupyter Notebook, e.g. cloud free ndvi.
   *
   ...
   *
   * @return results Info containing thematic mean aggregate png path, time, product type and tiff path
   */
  def mean(tileLayerRddWithMeta:(RDD[(SpaceTimeBandKey,Tile)],RasterTileLayerMetadata[SpaceTimeKey]),
...
//    val spatialTemporalBandRdd:RDD[(SpaceTimeKey, (String, Tile))] = tranTileLayerRddWithMeta._1
    ...
    var meanRdd:RDD[(String,Iterable[(SpatialKey,Tile)])] = bandAndSpatialTemporalRdd
    ...
    val results:RDD[Info] = meanRdd.map{x =>
...
//      val outputDir = "/home/geocube/tomcat8/apache-tomcat-8.5.57/webapps/html/"//"/home/geocube/environment_test/geocube_core_jar/"
      ...
      stitched.tile.renderPng(colorRamp).write(outputDir + uuid + "_mean_" + band + ".png")

      //generate ndvi thematic product
      val outputTiffPath = outputDir + uuid + "_mean_" + band + ".TIF"
      GeoTiff(stitched, crs).write(outputTiffPath)
//      val outputThematicPngPath = outputDir + uuid + "_mean_thematic" + band + ".png"
//      val stdout = new StringBuilder
//      val stderr = new StringBuilder
//      Seq("/home/geocube/qgis/run.sh", "-t", "mean", "-r", s"$outputTiffPath", "-o", s"$outputThematicPngPath") ! ProcessLogger(stdout append _, stderr append _)
  ...
  /**
   * This Mean function is used in web service and web platform.
   *
   * Use (RDD[(SpaceTimeBandKey,Tile)],RasterTileLayerMetadata[SpaceTimeKey]) as input.
   *
   * @param tileLayerRddWithMeta a rdd of queried tiles
...
//          reslist:+(y._1,tile)
  ...
  def main(args: Array[String]): Unit = {
    ...
    //mean
    val analysisBegin = System.currentTimeMillis()
    mean(tileLayerRddWithMeta,0.3,outputDir)
    mean(tileLayerRddWithMeta, outputDir)
    val analysisEnd = System.currentTimeMillis()
