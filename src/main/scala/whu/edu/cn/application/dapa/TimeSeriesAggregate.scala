...
import whu.edu.cn.core.entity.{QueryParams, RasterTileLayerMetadata, SpaceTimeBandKey,GcMeasurement}
...
object TimeSeriesAggregate {
  /**
    ...
    * @return tile
    */
  def generateMaxTile(TileRDDList:RDD[(SpatialKey, Tile)],metadata: TileLayerMetadata[SpaceTimeKey],bbox:String):Raster[Tile] = {
...
//    val rasterTileRdd = bandAndSpatialTemporalRdd.filter({x=>x._1.equals(bandName)}).groupBy(y=>y._2._1.instant)
    ...
    val temporalGroupRdd:RDD[(Long, Iterable[(SpaceTimeKey,Tile)])]  = bandAndSpatialTemporalRdd.filter({x=>x._1.equals(bandName)}).map(y=>(y._2._1.instant,y._2)).groupByKey()
//    val values:((Double,Double),Double) = generateValues(rasterTileRdd,srcMetadata,bbox)
    ...
    temporalGroupRdd.foreach{x =>
      ...
      println("<--------" + instantRet + ": " + requestedExtent + "-------->")
      ...
      val outputPath = executorOutputDir + bandName+ "_" + instantRet + ".png"
      tile.renderPng(colorRamp).write(outputPath)
      ...
      node.put("time", instantRet)
      node.put("field",bandName)
      node.put("min",tile.findMinMaxDouble._1)
      node.put("max",tile.findMinMaxDouble._2)
      node.put("average",average)
      node.put("extent", requestedExtent.xmin + "," + requestedExtent.ymin + "," + requestedExtent.xmax + "," + requestedExtent.ymax)
      objectMapper.writerWithDefaultPrettyPrinter().writeValue(new FileOutputStream(outputMetaPath), node)
      val scpMetaCommand1 = "scp " + outputMetaPath + " geocube@gisweb1:" + outputDir
