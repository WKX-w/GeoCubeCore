package whu.edu.cn.application.dapa
...
import whu.edu.cn.util.{GcConstant, TileUtil}
...
/**
 * Generate min fields value from bands.
 *
 *
 */
object MinAggregate {
  /**
    ...
    * find the min pixel of each tile
    * @param TileRDDList
    * @return tile
    */
  def generateMaxTile(TileRDDList:List[(SpaceTimeKey, Tile)]):Tile = {
    Min(TileRDDList.map({x=>x._2}))
  }
  /**
   * This Min function is used in Jupyter Notebook, e.g. cloud free ndvi.
   *
   ...
   *
   * @return results Info containing thematic min aggregate png path, time, product type and tiff path
   */
  def min(tileLayerRddWithMeta:(RDD[(SpaceTimeBandKey,Tile)],RasterTileLayerMetadata[SpaceTimeKey]),
...
//    val spatialTemporalBandRdd:RDD[(SpaceTimeKey, (String, Tile))] = tranTileLayerRddWithMeta._1
    ...
    var minRdd:RDD[(String,Iterable[(SpatialKey,Tile)])] = bandAndSpatialTemporalRdd
    ...
    val results:RDD[Info] = minRdd.map{x =>
...
//      val outputDir = "/home/geocube/tomcat8/apache-tomcat-8.5.57/webapps/html/"//"/home/geocube/environment_test/geocube_core_jar/"
      ...
      stitched.tile.renderPng(colorRamp).write(executorOutputDir + uuid + "_min_" + band + ".png")

      //generate ndvi thematic product
      val outputTiffPath = outputDir + uuid + "_min_" + band + ".TIF"
      GeoTiff(stitched, crs).write(outputTiffPath)
//      val outputThematicPngPath = outputDir + uuid + "_min_thematic" + band + ".png"
//      val stdout = new StringBuilder
//      val stderr = new StringBuilder
//      Seq("/home/geocube/qgis/run.sh", "-t", "min", "-r", s"$outputTiffPath", "-o", s"$outputThematicPngPath") ! ProcessLogger(stdout append _, stderr append _)
  ...
  /**
   * This Min function is used in web service and web platform.
   *
   * Use (RDD[(SpaceTimeBandKey,Tile)],RasterTileLayerMetadata[SpaceTimeKey]) as input.
   *
   * @param tileLayerRddWithMeta a rdd of queried tiles
...
//          reslist:+(y._1,tile)
    ...
    minRdd.foreach{x =>
      ...
      val outputPath = executorOutputDir + "Min_" + band + ".png"
  ...
  def main(args: Array[String]): Unit = {
    ...
    //min
    val analysisBegin = System.currentTimeMillis()
    min(tileLayerRddWithMeta,0.3,outputDir)
    min(tileLayerRddWithMeta, outputDir)
    val analysisEnd = System.currentTimeMillis()
