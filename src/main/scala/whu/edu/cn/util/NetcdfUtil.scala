...
import whu.edu.cn.application.gdc.gdcCoverage.{getCropExtent, scaleCoverage}
...
object NetcdfUtil {
  ...
  def ncWriteMeasurements(rasterTileRddWithDimensions: RDD[((ZonedDateTime, String, Seq[GcDimension]), Raster[Tile])],
    ...
    for (measurement <- measurementArray) {
      println("measurement: " + measurement)
      ncFile.setRedefineMode(true)
      val variable = ncFile.addVariable(measurement, DataType.DOUBLE, dimensionList)
      ...
      ncFile.setRedefineMode(false)
      ncFile.write(variable, variableArray)
      println("size: " + variableArray.getSize)
    }
  }
