...
import whu.edu.cn.core.entity.GcProduct.getDimensionCoordinates
import whu.edu.cn.core.entity.{GcDimension, VectorGridFact}
import whu.edu.cn.core.vector.grid.GridTransformer
...
class PostgresqlService {
  ...
  def getOtherDimensionNames(cubeId: String):ArrayBuffer[String] = {
    val sql = "select dimension_name " +
      "from gc_dimension_" + cubeId + " where dimension_name NOT IN ('extent', 'product', 'phenomenonTime', 'measurement')"
    val postgresqlUtil = new PostgresqlUtil(sql)
    val gcDimensionNameArray: ArrayBuffer[String] = new ArrayBuffer[String]()
    try {
      val resultSet = postgresqlUtil.getStatement().executeQuery()
      val columnCount = resultSet.getMetaData.getColumnCount;
      while (resultSet.next()) {
        for (i <- 1 to columnCount) {
          gcDimensionNameArray.append(resultSet.getString(i))
        }
      }
    } catch {
      case e: SQLException =>
