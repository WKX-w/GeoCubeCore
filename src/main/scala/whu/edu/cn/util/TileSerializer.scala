...
object TileSerializer {
  ...
  /**
   * Determines the byte size of a given data type.
   *
   * @param dataType The string representation of the data type.
   * @return The byte size corresponding to the provided data type. If the data type is not recognized,
   *         a default size of 8 bytes is returned.
   */
  def getByteSize(dataType: String): Int = {
    dataType match {
      case "int8raw" => 1
      case "uint8raw" => 1
      case "int8" => 1
