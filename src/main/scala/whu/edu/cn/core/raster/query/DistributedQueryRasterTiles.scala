package whu.edu.cn.core.raster.query
...
import whu.edu.cn.core.cube.raster.RasterRDD
import whu.edu.cn.core.entity
import whu.edu.cn.core.entity._
import whu.edu.cn.core.entity.GcProduct._
import whu.edu.cn.core.entity.GcMeasurement._
import whu.edu.cn.core.vector.grid.GridTransformer.getGeomGridInfo
import whu.edu.cn.util.HbaseUtil.{getTileCell, getTileMeta}
import whu.edu.cn.util.{GcConstant, PostgresqlService, PostgresqlUtil, TileSerializer, TileUtil}
import whu.edu.cn.util.TileSerializer.{deserializeTileData, getByteSize}
import whu.edu.cn.util.TileUtil.convertNodataValue
...
object DistributedQueryRasterTiles {
   ...
   */
  def getRasterTileRDD(implicit sc: SparkContext, p: QueryParams, olap: Boolean = false, isGetData: Boolean = false, format: String = "tif"): (RDD[(SpaceTimeBandKey, Tile)], RasterTileLayerMetadata[SpaceTimeKey]) = {
    if (p.getRasterProductNames.length == 0 || p.getRasterProductNames.length == 1) { //single product
      ...
      if (!olap) getRasterTileRDDWithMeta(sc, p, format, isGetData)
   ...
   */
  def getRasterTileRDDWithMeta(implicit sc: SparkContext, p: QueryParams, format: String = "tif", isGetData: Boolean = false): (RDD[(SpaceTimeBandKey, Tile)], RasterTileLayerMetadata[SpaceTimeKey]) = {
    var isTif = true
...
//        }
        ...
        // 如果是输出一张tif,则取第一组product
        var productKeys = new StringBuilder
        if (isTif && isGetData) {
          val productNewSql = getFirstGroupProduct(productsql, cubeId)
          println(productNewSql.toString())
          val productResults = statement.executeQuery(productNewSql.toString());
          if (productResults.first()) {
            // {1,2,3}-->('1','2','3')
            val regex = """\d+""".r
            val matchedNumbers = regex.findAllIn(productResults.getString(1)).toList
            // 将匹配到的数字用单引号包裹，并使用逗号连接
            productKeys = new StringBuilder("(" + matchedNumbers.map("'" + _ + "'").mkString(", ") + ")")
        ...
        val queryTileIDEnd = System.currentTimeMillis()
        println("-------- Returned " + tileAndDimensionKeys.length + " raster tiles, time cost is " + (queryTileIDEnd - queryTileIDBegin) + "ms --------")
        // check if resize the server provide
        if (checkExceedLimit(p, tileAndDimensionKeys.length, cubeId, productKeyArray(0), isGetData)) {
          return null
        }
        val postgresqlService = new PostgresqlService()
  ...
  /**
   * check if the total tile has exceeded the limit
   *
   * @param param      the data query params
   * @param tileNum    the number of the tiles
   * @param cubeId     the id of the cube
   * @param productKey the productkey
   * @return flag true is exceed false is not exceed
   */
  def checkExceedLimit(param: QueryParams, tileNum: Int, cubeId: String, productKey: String, isGetData: Boolean): Boolean = {
    var limitSize = GcConstant.limitByteSize
    if(!isGetData){
      limitSize = GcConstant.limitBytesSizeWorkflow
    }
    var flag: Boolean = false
    val SRE: ((Double, Double), (Double, Double, Double, Double)) = param.getSizeResAndExtentByCubeId(param.getCubeId, PostgresqlUtil.url, PostgresqlUtil.user, PostgresqlUtil.password).toList.head
    val tileSize: Double = SRE._1._2
    val postgresqlService = new PostgresqlService()
    val dType: String = postgresqlService.getCellType(cubeId, productKey)
    val byteSize: Int = getByteSize(dType)
    val totalByteSize: Long = (tileNum * tileSize * tileSize * byteSize).toLong
    if (totalByteSize > limitSize) {
      flag = true
    }
    flag
  }
  ...
  def getFirstGroupProduct(initSQL: StringBuilder, cubeId: String): StringBuilder = {
    val initString = initSQL.toString()
    val newSQL: StringBuilder = new StringBuilder(initString.replace("Select DISTINCT product_key from ", "SELECT array_agg(product_key) AS product_keys from "))
    val postgresqlService = new PostgresqlService()
    val gcDimensionNameArray: ArrayBuffer[String] = postgresqlService.getOtherDimensionNames(cubeId)
    newSQL ++= " GROUP BY "
    newSQL ++= gcDimensionNameArray.mkString(",") + ",phenomenon_time;"
    newSQL
  }
  ...
  (RDD[(SpaceTimeBandKey, Tile)], RasterTileLayerMetadata[SpaceTimeKey]) = {
    ...
    //    val newTl = TileLayout(((longLati(2) - longLati(0)) / SRE._1._1).toInt, ((longLati(3) - longLati(1)) / SRE._1._1).toInt, tileSize, tileSize)
    //    val newLd = LayoutDefinition(geotrellis.vector.Extent(longLati(0), longLati(1), longLati(2), longLati(3)), newTl)
    val minCol = colRow(0);
    val minRow = colRow(1);
    val maxCol = colRow(2);
    val maxRow = colRow(3)
    ...
    //    (layerRdd, RasterTileLayerMetadata(TileLayerMetadata(celltype, newLd, actualExtent, crs, bounds), productName))
  }
